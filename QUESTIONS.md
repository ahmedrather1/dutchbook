...
| Q5 | **Polymarket taker fee formula: two conflicting published forms.** Official docs say `fee = C × feeRate × p × (1−p)`; a secondary source says `fee = C × p × feeRate × (p(1−p))^exponent`. The official form is implemented. | They are not equivalent. If the exponent form is current, Chapter 5's numbers are wrong and `lib/venues/polymarket.ts` needs revising. | C-7, G-5 |
| Q5a | Kalshi items not found in the docs read on 2026-09-12: **tick size**, **maker fee**, **settlement fee**. Modelled as 1¢ tick, same fee for both roles, no settlement fee. | Same risk as Q5 — a wrong constant teaches a false lesson (D14). | C-2, G-5 |
| Q5b | Polymarket **minimum order size** not found in the page read. Not modelled. | Affects whether a small arb is actually executable (Chapter 6 sizing). | C-3, H-1 |
...
| Q10 | Polymarket's real 0.25¢ tick is not representable at our price resolution (1/1000 of a dollar), so the model uses 0.5¢. | Integer prices are non-negotiable (D20). Fixing it properly means raising `ONE_DOLLAR` to 10000, which churns every price in the codebase. Cheap now, expensive after twelve chapters of content exist. | C-3, H-2 |
